...
impl Gpio<Uninitialized> {
    ...
    pub fn into_alt(self, function: Function) -> Gpio<Alt> {
        ...
        let bit = ((self.pin % 10) * 3) as usize;
        let mut mask = self.registers.FSEL[reg].read() & !(0b111 << bit);

        mask |= (function as u32) << bit;
        self.registers.FSEL[reg].write(mask);
        self.transition()
    }
...
impl Gpio<Input> {
    ...
    pub fn level(&mut self) -> bool {
        self.registers.LEV[(self.pin / 32) as usize].read() & (1 << (self.pin % 32)) != 0
    }
}
