...
impl<T: io::Read + io::Write> Xmodem<T> {
    ...
    fn expect_byte(&mut self, byte: u8, expected: &'static str) -> io::Result<u8> {
        if self.read_byte(byte != CAN)? == byte {
            return Ok(byte);
        }
        ioerr!(InvalidData, expected)
    }
    ...
    pub fn read_packet(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        if buf.len() < 128 {
            return ioerr!(UnexpectedEof, "nice brain");
        }
        if !self.started {
            self.started = true;
            self.write_byte(NAK)?;
        ...
        if byte == SOH {
            if self.read_byte(false)? != self.packet || self.read_byte(false)? != 255 - self.packet {
                self.write_byte(CAN)?;
                return ioerr!(InvalidData, "This is invalid data you moron");
            }
            for i in 0..128 {
            ...
            if self.read_byte(false)? != get_checksum(&buf) {
                self.write_byte(NAK)?;
                return ioerr!(Interrupted, "What is this even supposed to be");
            }
            self.write_byte(ACK)?;
            ...
            self.write_byte(NAK)?;
            self.expect_byte_or_cancel(EOT, "This should be EOT")?;
            self.write_byte(ACK)?;
            return Ok(0);
        } else {
            return ioerr!(InvalidData, "This is invalid data you moron");
    ...
    pub fn write_packet(&mut self, buf: &[u8]) -> io::Result<usize> {
        if !self.started {
            ...
            self.expect_byte_or_cancel(NAK, "This should be a NAK")?;
            (self.progress)(Progress::Started);
        }
        if buf.len() == 0 {
            self.write_byte(EOT)?;
            self.expect_byte_or_cancel(NAK, "This should be a NAK")?;
            self.write_byte(EOT)?;
            self.expect_byte_or_cancel(ACK, "This should be an ACK")?;
            return Ok(0);
        }
        if buf.len() < 128 {
            return ioerr!(UnexpectedEof, "nice brain");
